#!/usr/bin/python3

import math


class InputError(Exception):
    pass


class SyntaxError(InputError):
    pass


class ConvertError(InputError):
    pass


def split_test(str_pos: str) -> list[str]:
    split_input = str_pos.split(",")
    if len(split_input) < 3:
        raise SyntaxError("Invalid syntax")
    return split_input


def convert_test(str_element: str) -> float:
    try:
        return float(str_element)
    except ValueError:
        raise ConvertError(f"Error on parameter '{str_element}': "
                           "could not convert string to float:  "
                           f"'{str_element}'")


def get_player_pos() -> list[float]:
    while True:
        input_pos = input("Enter new coordinates as "
                          "floats in format 'x,y,z': ")
        float_list: list[float] = []
        try:
            split_list = split_test(input_pos)
            for element in split_list:
                int_element = convert_test(element)
                float_list.append(int_element)
            return float_list
        except InputError as e:
            print(e)


def first_formula(tuple_list: tuple[float, ...]) -> None:
    x_1 = tuple_list[0]
    y_1 = tuple_list[1]
    z_1 = tuple_list[2]

    print(f"Got a first tuple: {tuple_list}")
    print(f"It includes: X={x_1}, Y={y_1}, Z={z_1}")
    distance = math.sqrt((0 - x_1)**2 + (0 - y_1)**2 + (0 - z_1)**2)
    print(f"Distance to center: {distance:.4f}")


def second_formula(tuple_one: tuple[float, ...],
                   tuple_two: tuple[float, ...]) -> None:
    x_1 = tuple_one[0]
    y_1 = tuple_one[1]
    z_1 = tuple_one[2]

    x_2 = tuple_two[0]
    y_2 = tuple_two[1]
    z_2 = tuple_two[2]
    distance = math.sqrt((x_2 - x_1)**2 + (y_2 - y_1)**2 + (z_2 - z_1)**2)
    print(f"Distance between the 2 sets of coordinates: {distance:.4f}")


if __name__ == "__main__":
    print("=== Game Coordinate System ===")
    print()
    print("Get a first set of coordinates")
    result = get_player_pos()
    tuple_result = tuple(result)
    first_formula(tuple_result)
    print()
    print("Get a second set of coordinates")
    result2 = get_player_pos()
    tuple_result2 = tuple(result2)
    second_formula(tuple_result, tuple_result2)
